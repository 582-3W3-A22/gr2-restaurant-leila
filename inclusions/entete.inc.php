<?php 
  ...
  // Inclure le fichier contenant le texte dans la langue demandée
  include("i18n/$lan/textes.php"); // La variable est interpolée (au lieu d'être concaténée)
?>
<!DOCTYPE html>
<html lang="<?= $lan ?>">
<head>
  ...
  <link href="https://fonts.googleapis.com/css2?family=Cinzel+Decorative:wght@700;900&family=Roboto+Slab:wght@300;700&family=Roboto:wght@400;700&display=swap" rel="stylesheet">
  <meta charset="UTF-8">
  ...
  <div id="conteneur" class="page-<?= $page; ?>">
    <header>
      <div class="barre-haut">
        ...
        <nav class="i18n">
          <!-- 
            Chaque langue disponible aura son lien (a) pour l'interactivité 
            et donc on peut générer ces liens en parcourant le tableau des 
            langues disponibles
          -->
          <!-- Début de la boucle -->
          <?php 
            for($i = 0; $i < count($languesDisponibles); $i++) { 
              $uneLangue = $languesDisponibles[$i];   
          ?>
          <a 
            href="?langue=<?= $uneLangue; ?>" 
            class="<?= $lan === $uneLangue ? 'actif' : ''; ?>" 
            title="">
            <?= $uneLangue; ?>
          </a>
          <?php } ?>
